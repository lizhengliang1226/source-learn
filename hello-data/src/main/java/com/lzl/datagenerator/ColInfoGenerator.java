...
public class ColInfoGenerator {
    ...
    public static final Map<Object, List<Object>> DICT_CACHE;
    ...
    static {
        ...
        // 加载字典缓存
        try {
            DICT_CACHE = Db.use().findAll("UPM_DICT_ITEMS")
                           .stream().collect(Collectors.groupingBy(entity -> entity.get("DICT_CODE"),
                                                                   Collectors.mapping(entity -> entity.get("DICT_ITEM"), Collectors.toList())));
    ...
    public static Object getDefaultValByColName(String colName) {
        ...
        if (val == null) {
            Optional<Object> patternVal = PATTERN_MAP.entrySet().stream().filter(e -> e.getKey().matcher(colName).find()).map(Map.Entry::getValue).findAny();
