...
public abstract class AbstractQueuedSynchronizer
        ...
        for (;;) {
            ...
            if (node == null) {                 // allocate; retry before enqueue
                ...
                // 把尾节点置为当前节点失败
                    ...
                    // 成功，则把尾节点后节点设置为当前节点，此时当前节点变成尾节点
                    t.next = node;
    ...
    private void cleanQueue() {
        ...
        for (;;) {                               // restart point
            // 从尾节点开始遍历！
            // q：尾节点(但其实代表当前节点的前节点)，s当前节点(初始是null，因为从尾节点开始遍历，当前是尾节点)，p，前节点，代表此时此刻的前节点，也就是前的前
            for (Node q = tail, s = null, p, n;;) { // (p, q, s) triples
                // 尾节点为空，或者尾节点的前节点为null，则没有可以取消的了，直接返回
                // 另一种理解：前节点为null或者前节点的前节点为null
                if (q == null || (p = q.prev) == null)
                    return;                      // end of list
                // s为null，第一次进来，
                //      尾节点不等于当前的尾节点q，则退出，状态不一致
                // s不为null，此时代表上次处理节点
                //      s的前节点不等于q(s是上次处理，q是上次处理的前节点s.prev!=q说明不一致了，照理应该是相等的)，或者状态已经是取消，则也直接结束
                if (s == null ? tail != q : (s.prev != q || s.status < 0))
                    break;                       // inconsistent
                // 前节点状态为取消
                if (q.status < 0) {              // cancelled
                    // s为null
                    //      第一次进来，q取消，则应该把p置为尾节点，所以casTail，q删除
                    // s不为null
                    //      后面进来，前节点取消，s是当前，p是前的前，此时q取消，则把当前的前置为前的前，所以casPrev
                    // q的前等于p，表示状态还不能被改变，要维持原样
                    if ((s == null ? casTail(q, p) : s.casPrev(q, p)) &&
                            q.prev == p) {
                        // 在上面cas成功后，则尾节点或者前节点已经被重置，此时要把前的前的后置为s，这是个双向链表
                ...
                if ((n = p.next) != q) {         // help finish
                    // 如果此时前的前的后的前等于前，说明前的前的后改变了，但后的前没变，需要将前的前的后改变的后纠正回当前的前节点
                    if (n != null && q.prev == p) {
                        p.casNext(n, q);
                        // 如果前节点为null
                        if (p.prev == null)
